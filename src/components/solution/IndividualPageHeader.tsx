...
const IndividualPageHeader = ({
  ...
  buttonLabel,
  ctaText,
}: IndividualPageHeaderProps) => {
  const navigate = useNavigate()
  const handleNavigate = ()=>{
      navigate('/activate-profile')    
  }
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 place-items-center md:gap-8 xl:gap-10 ">
      <div className="bg-glassBg rounded-lg">
        <h2 className="h mb-[30px] text-[50px] text-[#1A000F] sm:text-[17px] md:text-[40px] leading-[50px] w-[79%]">
          {heading}
        </h2>
